...
struct FlowLayout: Layout {
    ...
    /// An item wider than the row is offered the row width so its text
    /// wraps instead of running off the screen.
    private func size(of view: LayoutSubview, in width: CGFloat) -> CGSize {
        let ideal = view.sizeThatFits(.unspecified)
        if width > 0, ideal.width > width {
    ...
    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout Void) -> CGSize {
        ...
        for view in subviews {
            let size = size(of: view, in: width)
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout Void) {
        ...
        for view in subviews {
            let size = size(of: view, in: bounds.width)
            if x + size.width > bounds.maxX, x > bounds.minX { x = bounds.minX; y += rowHeight + spacing; rowHeight = 0 }
            let proposalForItem = size.width >= bounds.width
                ? ProposedViewSize(width: bounds.width, height: nil) : .unspecified
            view.place(at: CGPoint(x: x, y: y), proposal: proposalForItem)
            x += size.width + spacing
            rowHeight = max(rowHeight, size.height)
        }
    }
