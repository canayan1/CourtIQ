...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), SF Pro Display for all
  display + SF Pro Display body + JetBrains Mono for the index/code voice on a warm-navy code surface,
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "SF Pro Display", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "SF Pro Display", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "SF Pro Display", cqw: 2.3, weight: 500, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "SF Pro Display", cqw: 1.46, weight: 500, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "SF Pro Display", cqw: 1.35, weight: 500, tracking: "0.18em", upper: true }
  ...
  # — display ramp (SF Pro Display 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "SF Pro Display", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "SF Pro Display", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "SF Pro Display", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "SF Pro Display", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "SF Pro Display", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "SF Pro Display", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a SF Pro Display figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (SF Pro Display italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. SF Pro Display italic, small SF Pro Display uppercase cite beneath."
  section-rule:
...
- **Fonts** — already set to **SF Pro Display** (display) / **-apple-system** (body); ignore any preset font name lingering in prose.
...
Three editorial voices, each in its own face: **SF Pro Display** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **SF Pro Display** carries body, leads, card titles, buttons, and
...
- **SF Pro Display** (sentence case, negative-tracked) for all display; **SF Pro Display** body/chrome; **JetBrains Mono** index + code.
...
- **Squint** — one SF Pro Display display moment dominates at 3–6× its neighbor.
...
- **Type** — SF Pro Display sentence-case display (negative-tracked); SF Pro Display 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (SF Pro Display `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (SF Pro Display `headline` 4.6cqw
...
- **SF Pro Display display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **SF Pro Display body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — SF Pro Display figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — SF Pro Display italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized SF Pro Display · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line SF Pro Display `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** SF Pro Display 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single SF Pro Display line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line SF Pro Display `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a SF Pro Display `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: SF Pro Display italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a SF Pro Display **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small SF Pro Display uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** SF Pro Display italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short SF Pro Display sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **SF Pro Display, sentence case**, negative-tracked; **SF Pro Display 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a SF Pro Display figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case SF Pro Display display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one SF Pro Display display moment dominates.
...
- **Type** — SF Pro Display sentence-case display (negative-tracked); SF Pro Display 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
