...
| Doubles cilaları | (1) Partnerin göreceği isim: gerçek adı olmayan hesaplara davet/kabulde bir kez soruluyor ("Player" fallback'i bitti; EN/TR). (2) Bağlı eşleşme raporunda **"Tekrar analiz et"**. (3) **Doubles IQ köprüsü:** Doubles ana ekranı + her raporun altında, 29 soruluk doubles senaryo quizini açan kart (Train ile aynı istatistik kaydı). İki simülatörde uçtan uca doğrulandı. |

## Bilinen takipler (1.0.3 adayları)
- **Swing skoru trafik-ışığı (kod hazır, 5 Tem 2026):** Çıplak 0–100 sayısı tek başına okunmuyor ("58 iyi mi?"). Skor halkasının altına 3 kademeli renk rozeti — **Building / Solid / Sharp** (clay / gold / moss, palet-içi) + tek satır açıklama (renk tek başına anlam taşımasın → erişilebilirlik). Eşikler edge modelinin kendi ölçeğine kalibre (40–70 normal, 85+ ileri): Building <40, Solid 40–69, Sharp 70+ — yeşil ulaşılabilir, düşük kademe nadir + dürüst, ton büyüme-odaklı ("kötü" demiyoruz). Geçmiş satır rozetleri de aynı kademeye göre renkli. `SwingScoreTier` tek kaynak. Üç kademe de simülatörde doğrulandı. Commit 21bdf40. Sadece swing (doubles skoruna dokunulmadı). 1.0.3 build'ine girer.
- **Vücut-farkındalıklı koçluk (kod hazır, 4 Tem 2026):** (1) Swing edge fonksiyonuna `ADAPTATION_SAFETY` sistem bloğu — görünen vücut tipine/yaşa sessizce uyum sağla, asla yorum yapma; ağır/yaşlı/mobilitesi kısıtlı oyunculara yüksek eklem yükü reçete etme, düşük etkili alternatif ver. **Deploy edilmedi — sıradaki "deploy et"te** `GLOBAL_DAILY_CALL_CAP` secret'ı + `REQUIRE_ENTITLEMENT=true` ile birlikte gidecek. (2) Profil > Oyunun'da opsiyonel **"Fiziksel notlar"** çipleri (diz/omuz/bel/kalça/dirsek/el bileği) — `PlayerContext` üzerinden swing + maç analizine "PHYSICAL NOTES" satırı olarak akıyor; simülatörde seçim + kalıcılık doğrulandı. Bu ikisi 1.0.3 build'ine girer (1.0.2 incelemedeyken arşiv YOK).
